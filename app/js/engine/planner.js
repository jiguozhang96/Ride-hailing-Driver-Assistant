...
function optimizePathDP(windows, candidates, homeAnchor, dayKind, weather, city, profile, date) {
  ...
  // 跨天夜班：hour >= 24 的时段改用次日 date（热力模板 + 限行均按次日）
  const heat = candidates.map((z, j) => windows.map(w => {
    const d = w.start >= 24 ? new Date(date.getFullYear(), date.getMonth(), date.getDate() + 1) : date;
    let hs = heatScore(z, dayKindOf(d, profile.bigWeek), w.start, weather);
    const r = checkRestriction(profile.plateType, d, w.start, city);
...
export function buildDailyPlan(profile, date = new Date(), weather = 'clear') {
  ...
  let startH = profile.startHour ?? 7;
  let endH = profile.endHour ?? 22;
  if (endH <= startH) endH += 24;  // 跨天夜班（如 22:00 → 次日 06:00）
...
export function fmt(h) {
  ...
  return (day > 0 ? '次日' : '') + `${String(hh).padStart(2, '0')}:${String(mm).padStart(2, '0')}`;
}
